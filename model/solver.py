...
Sections:
    ...
    D. Pareto solver — epsilon constraint method (10 points)
...
# Tests all feasible technology combinations
# ============================================================
...
def run_diagnostic(user_inputs: dict,
    ...
    """
    Test all feasible technology combinations and return results.
 
    Parameters
    ----------
    user_inputs : dict
    time_limit  : solver time limit per combination (seconds)
            ...
            for mech in mech_list:
                ...
                if "Infeasible" not in stat and "ERROR" not in stat:
                    ...
                    results.append({
                        ...
                        "NAC": round(nac, 4), "GHG": round(ghg, 1),
                        "Status": stat
                    })
                else:
                    results.append({
...
def run_pareto(user_inputs: dict,
               ...
               gap: float = 0.02) -> dict:
    """
    Run the full Pareto optimization using epsilon constraint method.
 
    Parameters
    ----------
    user_inputs : dict
    n_pts       : number of Pareto points
    time_limit  : solver time limit per point (seconds)
    gap         : relative optimality gap
 
    Returns
    -------
    dict with keys: pareto_df, NAC_min, GHG_min, status, message
    """
    try:
        m, sets, params, vars, eqs = build_model(user_inputs)
        solver_opts = Options(time_limit=time_limit,
        ...
        # ── STEP 1: Anchor A — minimise NAC ──────────────────────────
        print("=" * 60)
        print("ANCHOR A — minimise NAC (no GHG constraint)")
        print("=" * 60)
        unfix_all(vars)
        ...
        conv_A, bio_A, mech_A, htlrec_A, gasupg_A, stb_A = get_config_labels(vars)
 
        print(f"  Status : {mdl_A.status}")
        print(f"  NAC    : {NAC_min:.4f} M$/yr")
        print(f"  GHG    : {GHG_at_A:.1f} t CO2-eq/yr  ← GHG_max")
        print(f"  Config : {conv_A}+{bio_A}+{mech_A} "
              f"[HTLrec={htlrec_A}|GasUpg={gasupg_A}|STB={stb_A}]")
 
        # ── STEP 2: Anchor B — minimise GHG ──────────────────────────
        print("\n" + "=" * 60)
        print("ANCHOR B — minimise GHG (no NAC constraint)")
        print("=" * 60)
        unfix_all(vars)
        ...
        conv_B, bio_B, mech_B, htlrec_B, gasupg_B, stb_B = get_config_labels(vars)
 
        print(f"  Status : {mdl_B.status}")
        print(f"  GHG    : {GHG_min:.1f} t CO2-eq/yr  ← GHG_min")
        print(f"  NAC    : {NAC_at_B:.4f} M$/yr")
        print(f"  Config : {conv_B}+{bio_B}+{mech_B} "
              f"[HTLrec={htlrec_B}|GasUpg={gasupg_B}|STB={stb_B}]")
 
        # ── STEP 3: Build epsilon grid ────────────────────────────────
        ...
        print(f"  GHG_max : {GHG_max_val:.1f}  GHG_min : {GHG_min_val:.1f}")
        print("=" * 60)
 
        # ── STEP 4: Epsilon constraint sweep ──────────────────────────
        ...
        for idx, eps_ghg in enumerate(eps_grid):
            ...
            nac_val = get_value(NAC)
            ghg_val = get_value(GHG_total_tpy)
            conv, bio, mech, htlrec, gasupg, stb = get_config_labels(vars)
 
            print(f"  {pt_label}  eps={eps_ghg:9.1f}  "
                  f"NAC={nac_val:7.3f}  GHG={ghg_val:8.1f}  "
                  f"[{conv}+{bio}+{mech}|HTLrec={htlrec}|"
                  f"GasUpg={gasupg}|STB={stb}]")
 
            pareto_rows.append({
                "Pareto_pt": pt_label,
                "eps_GHG":   round(eps_ghg, 1),
                "Conv":      conv,  "Bio": bio, "Mech": mech,
                "HTLrec":    htlrec, "GasUpg": gasupg, "STB": stb,
                "NAC":       round(nac_val, 4),
                "GHG":       round(ghg_val, 1),
                "CCAC":      round(get_value(CCAC),  4),
                "CCWC":      round(get_value(CCWC),  4),
                "CCINS":     round(get_value(CCINS), 4),
                "CCUC":      round(get_value(CCUC),  4),
                "CCLB":      round(get_value(CCLB),  4),
                "CCOC":      round(get_value(CCOC),  4),
                "CCRM":      round(get_value(CCRM),  4),
                "CDISP":     round(get_value(CDISP), 4),
                "CTIP":      round(get_value(CTIP),  4),
                "REV":       round(get_value(REV),   4),
                "CCTC":      round(get_value(CCTC),  4),
                "Status":    status,
            })
        ...
        print("=" * 60)
        print(feasible_df[[
            "Pareto_pt", "eps_GHG", "Conv", "Bio", "Mech",
...
def run_optimization(user_inputs: dict) -> dict:
    ...
    user_inputs : dict with keys:
        - Qf           : feed flow rate (kg/hr)
        - Tann         : annual operating hours
        - composition  : dict of {WATER, CBH, PRT, FAT, OTH, ASH, FIXED_CARBON}
        - C_elec       : electricity cost ($/kWh)
        - C_tip        : tipping fee ($/kg)
        - C_lbr        : labor cost ($/hr)
        - CRF          : capital recovery factor
        - price_CH4    : CH4 price ($/kg)
        - price_BIOCRUDE     : biocrude price ($/kg)
        - price_COMPOST      : compost price ($/kg)
        - price_ELECTRICITY  : electricity price ($/kWh)
        - price_BIOSOLIDS    : biosolids price ($/kg)
        - n_pareto_pts : number of Pareto points (default 10)
        - time_limit   : solver time limit per point (default 300)
        - gap          : optimality gap (default 0.02)
 
    Returns
    -------
    dict with keys: status, message, pareto_df, NAC_min, GHG_min
    """
    return run_pareto(
